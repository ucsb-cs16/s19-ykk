...
assigned: 2019-04-17 08:00:00.00-7
due: 2019-04-23 23:59:00.00-7
---


* Read through the entire lab, and identify parts that seem unclear to you. Ask the mentors for clarifications.

This is an **individual** lab, however, you are allowed to discuss the concepts with your labmates. 
...
* In your `~/cs16` directory, type the following commands, replacing Sam Gaucho with your name and sgaucho@cs.ucsb.edu with your email address.

```
   git config --global user.name "Sam Gaucho"

   git config --global user.email "sgaucho@cs.ucsb.edu"
```
...
## Step 1b: Create a repo on github in our class organization
...
```
git clone git@github.com:ucsb-cs16-w19/{{page.num}}_jgaucho.git
```
...
```
cp /cs/faculty/ykk/cs16/labs/{{page.num}}/* ./
```
...
git pull
git add .
git commit -m "Initial version of {{page.num}} files"
git push origin master
```

Go ahead and type them out on a terminal **from your git repo directory**. The above commands save a snapshot of your code on github. To check that this was done sucessfully open a web-browser and navigate to your repo on github. Then check to see that the starter code appears in your repo.

**Note**: Every time you add a new piece of logic to your code you should save a snapshot of the latest version of your code by issuing the commands: `git add ...`, `git commit ...` and `git push ...`. All the previous versions will be available to you as well and you have the option of reverting to older versions. As you go through the rest of this lab you will need to use these commands to keep track of the different versions of your code.
...
Your job now is to start edit the starT.cpp program, which has a function inside of it that is a "stub".  That function does NOT produce the correct output---it always just returns the string "stub".   
**You need to replace that code with a proper implementation of starT.**  
You can use the implementation of starL in the starL.cpp file as a model.

Compile your starT.cpp to the execuatable starT. Suppose we want your program to draw a T with width 3 and height 2, we will run your starT executable as follows:
